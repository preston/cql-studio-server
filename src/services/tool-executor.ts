...
export class ToolExecutor {
  ...
  /**
   * Execute fetch content tool (returns formatted string for LLM consumption)
   */
  ...
  /**
   * Execute SearXNG search tool (anonymous web search via user-configured instance)
   */
  ...
  /**
   * Execute SearXNG search tool (formatted string for LLM consumption)
   */
  ...
  /**
   * Execute search-then-fetch: SearXNG search then fetch full content for top N results.
   */
  ...
  /**
   * Execute search-then-fetch formatted: same as above but return single string for LLM.
   */
  ...
  /**
   * Execute batch fetch: fetch multiple URLs, return array of structured results.
   */
  private async executeBatchFetch(params: any): Promise<any[]> {
    const { urls } = params;
    if (!Array.isArray(urls) || urls.length === 0) {
      throw new Error('urls is required and must be a non-empty array of URL strings');
    }
    const list = urls.slice(0, 10).filter((u: unknown) => typeof u === 'string' && u.trim());
    if (list.length === 0) {
      throw new Error('urls must contain at least one non-empty URL string');
    }
  ...
  /**
   * Execute fetch metadata (lightweight: final URL, status, content-type, title/description/image from meta).
   */
  ...
  /**
   * Execute fetch content as Markdown.
   */
  ...
  /**
   * Execute get rate limit status (no params, does not consume tokens).
   */
